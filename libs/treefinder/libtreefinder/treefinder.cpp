...
TreeFinder::TreeFinder(images::Image<float> depthmap, float resolution)
    : depthmap(std::move(depthmap))
    , resolution(resolution)
{
    int blur_size = static_cast<int>(2.5 / resolution) * 2 + 1; // radius - 2.5 m
...
std::vector<std::pair<size_t, size_t>> TreeFinder::find_trees(size_t maxima_radius, size_t search_radius, ParaboloidConfig upper, ParaboloidConfig lower, size_t lower_offset) {
    ...
    auto local_maxima = find_local_maxima(depthmap, maxima_radius);
...
void util::draw_mark(::images::Image<unsigned char>& image, int64_t row, int64_t col, Color color, size_t size) {
    for (size_t i = std::max<int64_t>(row - size, 0), iEnd = std::min<size_t>(row + size + 1, image.height); i < iEnd; ++i) {
        for (size_t j = std::max<int64_t>(col - size, 0), jEnd = std::min<size_t>(col + size + 1, image.width); j < jEnd; ++j) {
            ...
            image(i, j, 1) = color.blue;
            image(i, j, 2) = color.green;
        }
    }
}
...
bool TreeFinder::is_local_maxima(const images::Image<float>& depthmap, size_t row, size_t col, size_t size) {
...
std::vector<std::pair<size_t, size_t>> TreeFinder::find_local_maxima(const images::Image<float>& depthmap, size_t size) {
    std::vector<std::pair<size_t, size_t>> result;
    for (size_t i = size; i < depthmap.height - size; ++i) {
        for (size_t j = size; j < depthmap.width - size; ++j) {
            if (is_local_maxima(depthmap, i, j, size)) {
